...
export async function POST() {
  ...
  if (!spotifyApi?.email || !spotifyApi?.accessToken) {
    return NextResponse.json({ error: "Invalid Spotify API credentials" }, { status: 400 })
  }
  ...
  if (isUserLocked) {
    const response = {
      userMessage: buildUserMessage(UserMessageType.USER_LOCKED, { playlistCount: playlistsData.body.total }),
    };
    return NextResponse.json(response, { status: 429 })
  }

  await queueService.enqueueJob(spotifyApi.email, spotifyApi.accessToken);
  const response = {
